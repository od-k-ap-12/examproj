...
void User::PassTest(list<User*>& Users, vector <string>& Categories, vector<Test>& Tests)
{
	...
	File.close();
	cout << endl << "Первое слово - категория. Второе - тема теста. Выберите тест (напишите тему теста): ";
	cin >> TestTitle;
	...
	File.close();
	for (int i = 0; i < 6; i++) {
		cout << "Введите ответ на вопрос " << i + 1;
		cin >> Answers[i];
	}
	...
	cout << endl << "Правильных ответов: " << RightAnswersCount << endl;
	cout << endl << "Оценка: " << RightAnswersCount * 2 << " В процентах: " << 100 / 6 * 6 << endl;
